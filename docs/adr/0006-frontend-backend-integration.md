...
- For Tier 3, run the real, unmodified FastAPI `app` on a background thread via `uvicorn.Server` (bound to a free localhost port, `tests/conftest.py::live_backend_url`), and point `api_client.BACKEND_URL` at it. `api_client` then makes genuine HTTP requests over a real socket — the strongest available integration signal — while Gemini calls are still mocked at the `vertexai.generative_models` boundary (mocks live in the same process/thread, so they apply regardless of which thread handles the request).
